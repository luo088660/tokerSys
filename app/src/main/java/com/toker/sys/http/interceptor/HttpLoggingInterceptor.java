...
package com.toker.sys.http.interceptor;
...
public class HttpLoggingInterceptor implements Interceptor {
    ...
    public void log(String message) {
        LogUtils.d("HttpLoggingInterceptor", message);
    }
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        try {
            ...
            LogUtils.d("HttpLoggingInterceptor","<-- HTTP FAILED: " + e);
            throw e;
        }
    ...
    private void logForRequest(Request request, Connection connection) throws IOException {
        LogUtils.d("HttpLoggingInterceptor","-------------------------------request-------------------------------");
        ...
        try {
            ...
            if (logHeaders) {
                ...
                for (int i = 0, count = headers.size(); i < count; i++) {
                    log("\tabc \t" + headers.name(i) + ": " + headers.value(i));
//                    if (headers.name(i).equals("do1-Token")){
//                    }
                }
    ...
    private Response logForResponse(Response response, long tookMs) {
        ...
        try {
            ...
            if (logHeaders) {
                ...
                LogUtils.d("HttpLoggingInterceptor", "headers:"+headers);
                Map map = new HashMap();
                for (int i = 0, count = headers.size(); i < count; i++) {
                    ...
                    log(name + ": " + value);
                    LogUtils.d("HttpLoggingInterceptor", "AppApplication.TokenId:"+name);
                    LogUtils.d("HttpLoggingInterceptor", "AppApplication.TokenId:"+value);
                    if (name.toLowerCase().equals("do1-token")){
                        Tool.commit(AppApplication.getInstance(),"do1-Token",headers.value(i));
                    }
                }
                LogUtils.d("HttpLoggingInterceptor", "logForRequest: $map"+map);
//                FileUtils.setData(map,"T_token.txt");
                log(" ");
