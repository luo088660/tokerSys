 ...
 * 拨打电话
 */
class CallPreDialog(val mContext: Context) : View.OnClickListener {
    ...
    constructor(mContext: Context,isType: Int,content:String):this(mContext){
        this.isType = isType
        tvPhone?.text =if (isType == 1){
            "未获取位置信息。确认签到后，考勤将视为异常考勤，且不允许再次签到，确认签到吗？"
        }else if(isType == 2){
            "未获取位置信息。确认签退后，考勤将视为异常考勤，且不允许再次签退，确认签退吗？"
        }else{
            Html.fromHtml(String.format(mContext.resources.getString(R.string.tip_task_number_11),content))
        }
    }
    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            ...
            R.id.btn_call_phone_determine -> {
                    EventBus.getDefault().post(CustomerInvalid(99,"",isType))
