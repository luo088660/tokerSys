...
class TApprovalAdapter (
    val context: Context,
    var mBeans: MutableList<PageData>
) : BaseAdapter<PageData,TApprovalAdapter.TAViewHodler>(context, mBeans) {

    override fun onCreateViewHolder(p0: ViewGroup, p1: Int): TAViewHodler {
        if (mHeaderView != null && p1 == TYPE_HEADER) {
            return TAViewHodler(mHeaderView!!)
        }
        val view = LayoutInflater.from(context).inflate(R.layout.layout_t_entire_02, null)

        return TAViewHodler(view)
    }


    @SuppressLint("ResourceAsColor")
    @RequiresApi(Build.VERSION_CODES.M)
    override fun onBindViewHolder(p0: TAViewHodler, p1: Int) {
    ...
    inner class TAViewHodler(v: View) : RecyclerView.ViewHolder(v) {
