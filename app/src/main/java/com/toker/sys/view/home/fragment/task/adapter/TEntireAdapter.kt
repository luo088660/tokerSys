...
class TEntireAdapter(
    val context: Context,
    var mBeans: MutableList<PageData>) : BaseAdapter<PageData,TEntireAdapter.TEViewHodler>(context,mBeans) {
    override fun onCreateViewHolder(p0: ViewGroup, p1: Int): TEViewHodler {
        if (mHeaderView != null && p1 == TYPE_HEADER) {
            return TEViewHodler(mHeaderView!!)
        }
        val view = LayoutInflater.from(context).inflate(R.layout.layout_t_entire_02, null)

        return TEViewHodler(view)
    }

    @SuppressLint("ResourceAsColor")
    @RequiresApi(Build.VERSION_CODES.M)
    override fun onBindViewHolder(p0: TEViewHodler, p1: Int) {
        ...
        p0.pb04.progress = (curTurnover.toDouble()*100).toInt()
        p0.pb04.max = (data.turnover*100).toInt()
    ...
    inner class TEViewHodler(v: View) : RecyclerView.ViewHolder(v) {
