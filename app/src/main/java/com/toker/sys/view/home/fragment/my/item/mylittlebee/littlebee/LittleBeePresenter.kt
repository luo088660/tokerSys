package com.toker.sys.view.home.fragment.my.item.mylittlebee.littlebee

import android.os.Bundle
import com.toker.sys.mvp.base.BasePresenter
import com.toker.sys.utils.tools.GsonUtil
import com.toker.sys.view.home.fragment.my.item.bean.LittleBeeBean
import org.json.JSONObject

/**
 * @author yyx
 */

class LittleBeePresenter : BasePresenter<LittleBeeContract.View>(), LittleBeeContract.Presenter {
    override fun onSuccessData(jsonObject: JSONObject, url_type: Int, load_type: Int, bundle: Bundle?) {
        val tojson = jsonObject.toString()

        val toBean = GsonUtil.GsonToBean(tojson, LittleBeeBean.LittleBeBean::class.java)
        if (toBean?.isSuccess()!!) {
            mView?.showData(toBean.data)
        }


    }

    override fun loadRepositories() {
        reqData(1)
    }
}