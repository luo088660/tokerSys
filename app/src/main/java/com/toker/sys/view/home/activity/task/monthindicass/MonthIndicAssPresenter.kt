package com.toker.sys.view.home.activity.task.monthindicass

import android.os.Bundle
import com.toker.sys.mvp.base.BasePresenter
import com.toker.sys.utils.tools.GsonUtil
import com.toker.sys.view.home.activity.task.bean.MonthLBean
import org.json.JSONObject

/**
 * @author yyx
 */

class MonthIndicAssPresenter : BasePresenter<MonthIndicAssContract.View>(), MonthIndicAssContract.Presenter {
    override fun onSuccessData(jsonObject: JSONObject, url_type: Int, load_type: Int, bundle: Bundle?) {

        val toJson = jsonObject.toString()
        print("数据 --->$toJson")
        val toBean = GsonUtil.GsonToBean(toJson, MonthLBean.MBean::class.java)
        if (toBean!!.isSuccess()){
            mView?.showData(toBean.data)
        }

    }

    override fun loadRepositories() {
        reqData(1)
    }
}