 ...
 * 2) 接收不到自定义消息
 */
class JPReceiver : BroadcastReceiver() {

    override fun onReceive(context: Context, intent: Intent) {
        try {
            val bundle = intent.extras
            Logger.d(TAG, "[JPReceiver] onReceive - " + intent.action + ", extras: " + printBundle(bundle!!))

            if (JPushInterface.ACTION_REGISTRATION_ID == intent.action) {
                val regId = bundle.getString(JPushInterface.EXTRA_REGISTRATION_ID)
                Logger.d(TAG, "[JPReceiver] 接收Registration Id : " + regId!!)
                //send the Registration Id to your server...

            } else if (JPushInterface.ACTION_MESSAGE_RECEIVED == intent.action) {
                Logger.d(TAG, "[JPReceiver] 接收到推送下来的自定义消息: " + bundle.getString(JPushInterface.EXTRA_MESSAGE)!!)
                processCustomMessage(context, bundle)

            } else if (JPushInterface.ACTION_NOTIFICATION_RECEIVED == intent.action) {
                Logger.d(TAG, "[JPReceiver] 接收到推送下来的通知")
                val notifactionId = bundle.getInt(JPushInterface.EXTRA_NOTIFICATION_ID)
                Logger.d(TAG, "[JPReceiver] 接收到推送下来的通知的ID: $notifactionId")

            } else if (JPushInterface.ACTION_NOTIFICATION_OPENED == intent.action) {
                Logger.d(TAG, "[JPReceiver] 用户点击打开了通知")
...
//                context.startActivity(i)
                ...
                Logger.d(TAG, "[JPReceiver] 用户收到到RICH PUSH CALLBACK: " + bundle.getString(JPushInterface.EXTRA_EXTRA)!!)
                ...
                Logger.w(TAG, "[JPReceiver]" + intent.action + " connected state change to " + connected)
            } else {
                Logger.d(TAG, "[JPReceiver] Unhandled intent - " + intent.action!!)
            }
        } catch (e: Exception) {
...
//        if (MainActivity.isForeground) {
          ...
          Logger.d(TAG,"接收到的数据--->$message")
          Logger.d(TAG,"接收到的数据--->$extras ")
