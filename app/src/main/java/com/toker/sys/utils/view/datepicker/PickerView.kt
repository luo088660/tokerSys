 ...
 * 修改时间：2018/12/21
 */
class PickerView(private val mContext: Context, attrs: AttributeSet) : View(mContext, attrs) {
    ...
    private class ScrollTimerTask constructor(handler: Handler) : TimerTask() {
    ...
    private class ScrollHandler constructor(view: PickerView) : Handler() {
        private val mWeakView: WeakReference<PickerView>
    ...
    private fun initPaint() {
        ...
        mLightColor = ContextCompat.getColor(mContext, R.color.c_black)
    ...
    private fun drawText(canvas: Canvas, textColor: Int, offsetY: Float, text: String) {
        ...
        scale = if (scale < 0) 0.toFloat() else scale
        ...
        if (abs(mScrollDistance) < AUTO_SCROLL_SPEED) {
            ...
            if (mTimerTask != null) {
                ...
                if (mOnSelectListener != null && mSelectedIndex < mDataList.size) {
                    LogUtils.d("PickerView", "mDataList[mSelectedIndex]:${mDataList[mSelectedIndex]} ");
